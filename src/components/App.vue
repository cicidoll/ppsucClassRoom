<template>
  ...
  <div id="app" v-cloak>
    ...
    <div id="body-content">
      <div class="block">
        <el-carousel trigger="click" height="20vh">
          <el-carousel-item v-for="imageItem in carouselImageUrl" :key="imageItem.id" class="carouselItem">
    ...
    <div id="buildingRadio">
      <el-radio-group v-model="buildingName" @change="selectBuilding">
        <el-radio-button label="zhuJian">铸剑楼</el-radio-button>
        <el-radio-button label="zhongLou">中楼</el-radio-button>
        <el-radio-button label="XiPei">西配</el-radio-button>
      </el-radio-group>
    </div>
...
export default {
  data () {
    return {
      ...
      // 5、首页走马灯设置
      carouselImageUrl: [] // 先加载json文件，再加载图片
    }
  },
  components: {
    dayRadio,
  ...
  methods: {
    getClassRoomData () {
      ...
      axios.get(apiClassRoomUrl)
        .then((response) => {
          // handle success
          this.allRoomAllDayData = response['data']
        })
        .then(() => {
          // 使用初始化定义的建筑物变量与日期变量
          this.selectBuilding(this.buildingName)
        })
        .catch((error) => {
          // handle error
          console.log(error)
      ...
      axios.get(mobilizeBorrowUrl)
        .then((response) => {
          ...
          this.carouselImageUrl = [
            {id: 0, url: 'https://www.ppsuc.edu.cn/images/20201105081018.jpg'},
            {id: 1, url: 'https://www.ppsuc.edu.cn/images/ss1.jpg'},
            {id: 2, url: 'https://www.ppsuc.edu.cn/images/06421646-c30a-4bb0-8e0c-ae9dd4af5363.jpg'}
          ]
        })
        .catch((error) => {
          // handle error
    ...
    selectBuilding (name) {
      ...
      this.roomAllDayData = this.allRoomAllDayData[name]
